...
struct Options
{
	...
	int     option_r;
	int     threads;
	int     min_cluster_size; // min cluster size for select cluster
	int     min_seeds_num; // min seeds num for one cluster
	int     multiple; // the multiple * std
	size_t  max_entries;
	...
	Options() {
		...
		max_sequences = 1 << 20;
		mem_limit = 100000000;
		min_cluster_size = 25; // min size to generate seeds
		min_seeds_num = 5;
		multiple = 3;
		threads = 50000;
	};
	...
	bool SetOptions(int argc, const char *argv[], bool twodata = false, bool est = false);
...
struct Sequence
{
	...
	int   index_sorted; // index of the sequence after sorted 
	short state;
	int   cluster_id;
	float identity;
	float distance;
	int   coverage[4];
	bool  cluster_added; // judge sequence is added to cluster or not
						 // to ensure just add once
	...
	void ConvertBases();
	void Set_cluster_added(bool added);
	void SwapIn();
	void SwapOut();
...
public:
	...
	Vector<Sequence*>  sequences;
	Vector<int>        rep_seqs;
	Vector<Vector<int> > clusterIS; // store the cluster index after sorted
	Vector<Vector<int> > seedsIs; // store the seeds index in the sorted seq
	Vector<short int>  updated; // the seeds update or not
	Vector<float>      meanIS; //store the mean identify
	Vector<float>	   stdIS; // store the STD
	...
	void ClusterOne(Sequence *seq, int id, WordTable & table,
		WorkingParam & param, WorkingBuffer & buf, const Options & options, int preclusterOrNot);
	...
	int  CheckOne(Sequence *seq, WordTable & tab, WorkingParam & par, WorkingBuffer & buf, const Options & opt, int preclusterOrNot);
	int  CheckOneEST(Sequence *seq, WordTable & tab, WorkingParam & par, WorkingBuffer & buf, const Options & opt);
	int  CheckOneAA(Sequence *seq, WordTable & tab, WorkingParam & par, WorkingBuffer & buf, const Options & opt, int preclusterOrNot);
	float seq_align_my(Sequence *seq1, Sequence *seq2);
	float standard_deviation(Vector<float> a, float meann);
	void update_seeds(const Options & options);
	void Write_seqDB_to_clusters(const Options & options);
};
...
float current_time();
void MergeSortRecursionAndIndex_with_matrix_point(float * A, unsigned int left, unsigned int right, unsigned int * index);
