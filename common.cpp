...
int print_usage() {
	//cout << "Usage: ./db -i input file, -o output file name, -c cluster cutoff, -k number of kmer (default 10), -T CPU number (default 10)" << endl;
	cout << "\n=====================================================================\n" << endl;
	cout << "Uasge example: ./dmsc -i seq.fa -c 0.97 -o seq_dmsc97.txt" << endl << endl;
	cout << "-----Required:" << endl;
	cout << "-i     input file (e.g. -i seqs.fasta)." << endl;
	cout << "-o     output file (e.g. -o dmsc97.txt)." << endl;
	cout << "-c     cluster similarity threshold (e.g. -c 0.97), similarity = 1 - distance." << endl;
	cout << " \n----Optional:" << endl;
	cout << "-min   minimum cluster size for multi-core sequences selection (default: 25)." << endl;
	cout << "-tstd  the multiple of standard deviation (default: 3)." << endl;
	//cout << "-ms    match score in the sequence alignment (default: 2)." << endl;
	//cout << "-ss    substitution score in the sequence alignment (default: -2)." << endl;
	//cout << "-gs    gap score in the sequence alignment (default: -2)." << endl;
	//cout << "-max   maximum core sequences number for a cluster to save time(default: 10)." << endl;
...
bool Options::SetOptionCommon(const char *flag, const char *value)
{
	...
	//else if (strcmp(flag, "-M" ) == 0) max_memory  = atoll(value) * 1000000;
	...
	else if (strcmp(flag, "-min") == 0) min_cluster_size = intval;
	else if (strcmp(flag, "-tstd") == 0) multiple = intval;
		...
		if (threads > cpu) {
			threads = cpu;
			printf("Warning: total CPUs number: %i\n", cpu);
		}
		if (threads == 0) {
			threads = cpu;
			printf("total CPUs number: %i\n", cpu);
		}
		if (threads != intval) printf("Actual CPUs number used: %i\n\n", threads);
#else
...
bool Options::SetOptions(int argc, const char *argv[], bool twod, bool est)
{
	int i, n;
	char date[100];
	...
	printf("===============================================\n");
	//printf("Program: DMSC, V" DMSC_VERSION WITH_OPENMP ", %s, " __TIME__ "\n", date);
	//printf("Command:");
	n = 9;
	for (i = 0; i<argc; i++) {
		//printf( "\n         %s", argv[i] );
...
void Sequence::Set_cluster_added(bool added) {
	cluster_added = added;
}
...
void SequenceDB::Write_seqDB_to_clusters(const Options & options) {
	...
	int totall = 0;
	int M = rep_seqs.size();
	int M1 = clusterIS.size();
	int original_index;
	string db_clstr2 = strcat(aaa2, ".clstr_index");
	FILE *fout2 = fopen(aaa1.c_str(), "w+");
	for (int i = 0; i < M; i++) {
		for (int k = 0; k < clusterIS[i].size(); k++) {
			original_index = sequences[clusterIS[i][k]]->index;
			totall++;
			fprintf(fout2, "%i ", original_index);
		}
		//cout<<"i="<<i<<endl;
		fprintf(fout2, "\n");
	}
	printf("Total %d seqeunces clustered with %d OTUs.\n", totall, M);
	fclose(fout2);
}
...
void SequenceDB::WriteExtra1D(const Options & options)
{
	...
	string db_clstr_bak2 = strcat(aaa2, ".bak.clstr.index");
	...
	if (options.backupFile) {
		...
		fout2 = fopen(db_clstr_bak2.c_str(), "w+");
		for (i = 0; i<N; i++) {
			Sequence *seq = sequences[sorting[i] & 0xffffffff];
...
void SequenceDB::ClusterOne(Sequence *seq, int id, WordTable & table,
	WorkingParam & param, WorkingBuffer & buffer, const Options & options, int preclusterOrNot)
{
	...
	int flag = CheckOne(seq, table, param, buffer, options, preclusterOrNot);
	if (flag == 0) {
		if ((seq->identity>0) && (options.cluster_best)) {
			...
			Vector<int> new_cluster2;
			new_cluster.Append(id);
			clusterIS.Append(new_cluster);
			meanIS.Append(0.0);
			stdIS.Append(0.0);
			updated.Append(0);
			seedsIs.Append(new_cluster2);
			seq->cluster_id = size;//序列所属类别
			seq->identity = 0;
			seq->state |= IS_REP;   //
			seq->Set_cluster_added(true);
		...
		clusterIS[seq->cluster_id].Append(id);
		seq->Set_cluster_added(true);
	}
	if ((id + 1) % 1000 == 0) {
...
void SequenceDB::DoClustering(int T, const Options & options)
{
	...
	for (i = 0; i<N; ) {
		...
		//printf( "m = %i  %i,  %i\n", i, m, m-i );
		if (i == 0)
			printf("\r--- first generate clusters from %8i to %8i\n", i, m);
		else
			//printf("\r--- generate new clusters from %9i to %9i\n", i, m);
			if (last_table.size) {
				if (flagggg == 1) {
					printf("\r    seeds updated\n");
					printf("    assign remaining sequences to current seeds\n");
					printf("    assigned done\n");
					printf("    seeds updated\n");
					flagggg = 0;
				}
				printf("\r--- then  generate clusters from %8i to %8i\n", i, m);
				int print = (m - i) / 20 + 1;
#pragma omp parallel for schedule( dynamic, 1 )
				for (int j = i; j<m; j++) {
					...
					CheckOne(seq, last_table, params[tid], buffers[tid], options, 1);
					if (options.store_disk && (seq->state & IS_REDUNDANT)) seq->SwapOut();
					if (j%print == 0) {
						//printf("."); fflush(stdout);
					}
				}
				// after check, should update the clusters, seeds, means and std
				// update the clusters vector
				for (int jjj = i; jjj < m; jjj++) {
					Sequence *seq = sequences[jjj];
					if (seq->state & IS_REDUNDANT) {
						if (seq->cluster_added)
							continue;
						clusterIS[seq->cluster_id].Append(jjj);
						seq->Set_cluster_added(true);
					}
				}

				update_seeds(options);
				printf("\r    seeds updated\n");
				printf("\r    assign remaining sequences to current seeds\n");
				int may_stop = 0;
				int self_stop = 0;
				float p0 = 0;
				...
				for (int ks = i; ks<m; ks++) {
					...
					i = ks + 1;
					if (seq->state & IS_REDUNDANT) continue;
					ClusterOne(seq, ks, word_table, params[tid], buffers[tid], options, 1);
...
#pragma omp flush (stop)
					if (!stop) {
						if (j + 1 == N) may_stop = 1;
						if (0) { // use m0 to avoid other iterations satisfying the condition:
							//int tid = omp_get_thread_num();
							//for (int ks = i; ks<m; ks++) {
							//	Sequence *seq = sequences[ks];
							//	i = ks + 1;
...
#pragma omp flush (stop)
				...
				// after check, should update the clusters, seeds, means and std
				// update the clusters vector
				printf("\r    assigned done\n");
				for (int jjj = m0; jjj < N; jjj++) {
					Sequence *seq = sequences[jjj];
					if (seq->state & IS_REDUNDANT) {
						if (seq->cluster_added)
							continue;
		...
		if (i == start || m == N) {
			if (m == N) {
				printf("\r--- finally cluster remaining sequences\n");
			}
...
#pragma omp parallel for schedule( dynamic, 1 )
				for (kk = k; kk<mm; kk++) {
					...
					if (seq->state & IS_REDUNDANT) continue;
					int tid = omp_get_thread_num();
					CheckOne(seq, word_table, params[tid], buffers[tid], options, 1);
					if (options.store_disk && (seq->state & IS_REDUNDANT)) seq->SwapOut();
				}
				for (int jjj = k; jjj < mm; jjj++) {
					Sequence *seq = sequences[jjj];
					if (seq->state & IS_REDUNDANT) {
						if (seq->cluster_added)
							continue;
				...
				for (int ks = k; ks<mm; ks++) {
					...
					i = k = ks + 1;
					if (seq->state & IS_REDUNDANT) continue;
					ClusterOne(seq, ks, word_table, params[0], buffers[0], options, 1);
					bk = true;
					if (options.store_disk && (seq->state & IS_REDUNDANT)) seq->SwapOut();
					if (word_table.size >= max_items) break;
					int tmax = max_seqs - (frag_size ? seq->size / frag_size + 1 : 0);
				...
				if (word_table.size > 0) {
					//printf("    generate %8i new clusters\n", word_table.sequences.size());
					update_seeds(options);
					//printf("    seeds updated\n");
				}
				if (bk)
			...
			//printf("\r--- remaining sequences number:　%6i \n", m - i);
		}
		//printf("---------- new clusters: %8i \n", word_table.sequences.size());
	...
	//printf("\n--- %9i  sequences finished,  %9i  clusters\n", sequences.size(), rep_seqs.size());
...
// sort matrix 
void MergeSortRecursionAndIndex_with_matrix_point(float * A, unsigned int left, unsigned int right, unsigned int * index)    // 递归实现的归并排序(自顶向下)
{
...
void MergeAndIndex_with_matrix_point(float * A, unsigned int left, unsigned int mid, unsigned int right, unsigned int * idx) {
	int len = right - left + 1;
	float *temp = new float[len];       // 辅助空间O(n)
...
void SequenceDB::update_seeds(const Options & options) {
	int nnnnnn, size_is, min_size, indexxx, indexxx2;
	float stdd = 0.0, simm, sim_sum;
	for (int h = 0; h < rep_seqs.size(); h++) {
		if (updated[h] == 0 && clusterIS[h].size() > options.min_cluster_size) {
			//build a matrix to store the similarity and select the core sequences
			min_size = options.min_cluster_size;
			float **matrix;
			matrix = new float*[min_size];
			float *sim_sum_matrix = new float[min_size]();//store the sim sum of each sequence
			unsigned int *sim_sum_index = new unsigned int[min_size]();//store the index of each sequence
			for (int j = 0; j < min_size; j++) {
				matrix[j] = new float[min_size](); //这个指针数组的每个指针元素又指向一个数组
			}
			for (int vv = 0; vv < min_size - 1; vv++) {
				int index1 = clusterIS[h][vv];
				for (int nn = vv + 1; nn < min_size; nn++) {
					int index2 = clusterIS[h][nn];
					simm = seq_align_my(sequences[index1], sequences[index2]);
					matrix[vv][nn] = simm;
					matrix[nn][vv] = simm;
				}
			}
			Vector<float> sim_array2;
			float meannn2 = 0.0;
			size_is = options.min_seeds_num;
			nnnnnn = size_is * (size_is - 1);
			nnnnnn = nnnnnn / 2;
			for (int hehe = 0; hehe < options.min_seeds_num - 1; hehe++) {
				for (int hehe2 = hehe + 1; hehe2 < options.min_seeds_num; hehe2++) {
					simm = matrix[hehe][hehe2];
					sim_array2.Append(simm);
					meannn2 += simm;
				}
			}
			meannn2 = meannn2 / nnnnnn;
			stdd = standard_deviation(sim_array2, meannn2);
			updated[h] = 1;
			meanIS[h] = meannn2;
			stdIS[h] = stdd;
			/*
			for (unsigned int xx = 0; xx < min_size; xx++) {
				sim_sum = 0;
				for (int yy = 0; yy < min_size; yy++) {
					sim_sum += matrix[xx][yy];
				}
				sim_sum_matrix[xx] = sim_sum;
				sim_sum_index[xx] = xx;
			}
			// then find the core sequences
			MergeSortRecursionAndIndex_with_matrix_point(sim_sum_matrix, 0, min_size - 1, sim_sum_index);
			size_is = options.min_seeds_num;
			Vector<float> sim_array;
			float meannn = 0.0;
			nnnnnn = size_is * (size_is - 1);
			nnnnnn = nnnnnn / 2;
			for (int vv = 0; vv < size_is; vv++) {
				indexxx = sim_sum_index[min_size - 1 - vv];
				int index1 = clusterIS[h][indexxx];
				seedsIs[h].Append(index1);
				for (int uu = vv + 1; uu < size_is; uu++) {
					indexxx2 = sim_sum_index[min_size - 1 - uu];
					//int index2 = clusterIS[h][indexxx2];
					simm = matrix[indexxx][indexxx2];
					sim_array.Append(simm);
					meannn += simm;
				}
			}
			meannn = meannn / nnnnnn;
			stdd = standard_deviation(sim_array, meannn);
			updated[h] = 1;
			meanIS[h] = meannn;
			stdIS[h] = stdd;
			*/
			delete sim_sum_matrix;
...
int SequenceDB::CheckOne(Sequence *seq, WordTable & table, WorkingParam & param, WorkingBuffer & buf, const Options & options, int preclusterOrNot)
{
	int len = seq->size;
	param.len_upper_bound = upper_bound_length_rep(len, options);
	if (options.isEST) return CheckOneEST(seq, table, param, buf, options);
	return CheckOneAA(seq, table, param, buf, options, preclusterOrNot);
}
int SequenceDB::CheckOneAA(Sequence *seq, WordTable & table, WorkingParam & param, WorkingBuffer & buf, const Options & options, int preclusterOrNot)
{
	...
	int lens;
	int has_aa2 = 0;
	int rep_seq_cluster_index;
	//int continue_or_not;
	IndexCount *ic = lookCounts.items;
	ic = lookCounts.items;
	float mean_sim_seeds = 0.0;
	for (; ic->count; ic++) {
		if (!frag_size) {
			indexMapping[ic->index] = 0;
			if (preclusterOrNot == 1) { // if precluster, do original what do
				if (ic->count < required_aan - 20)
					continue;
			}
			if (preclusterOrNot == 0) {
				Sequence *reppp = table.sequences[ic->index];
				int cluster_index = reppp->cluster_id;
				if (updated[cluster_index] == 0) {
					if (ic->count < required_aan - 20)
						continue;
				}
			}
		...
		rep_seq_cluster_index = rep->cluster_id;
		if (preclusterOrNot == 0) { // if not precluster, that is assign, compare with seeds
			if (updated[rep_seq_cluster_index] == 1) {

				for (int aaa = 0; aaa < seedsIs[rep_seq_cluster_index].size(); aaa++) {
					int seed_index_each = seedsIs[rep_seq_cluster_index][aaa];
					Sequence *seed_seq = sequences[seed_index_each];
					float identif = seq_align_my(seed_seq, seq);
					mean_sim_seeds += identif;
				}
				mean_sim_seeds /= seedsIs[rep_seq_cluster_index].size();
				if (mean_sim_seeds >= options.cluster_thd) {
					seq->cluster_id = rep->cluster_id;
					flag = 1;
					break;
				}
				if (abs(mean_sim_seeds - options.cluster_thd) <= stdIS[rep_seq_cluster_index] * options.multiple) {
					seq->cluster_id = rep->cluster_id;
					flag = 1;
					break;
				}
		...
		//pre if (best_sum < required_aa2) continue;
		...
		//pre if (tiden_no < required_aa1) continue;
		lens = len;
		...
		float simm_average = -1.0;
		if (tiden_pc < options.cluster_thd && clusterIS[rep_seq_cluster_index].size() > 0) {
			int haha = min(clusterIS[rep_seq_cluster_index].size(), options.min_seeds_num);
			float simm;
			simm_average = 0.0;
			for (int jjj = 0; jjj < haha; jjj++) {
				int member = clusterIS[rep_seq_cluster_index][jjj];
				Sequence *seed_seq = sequences[member];
				simm = seq_align_my(seq, seed_seq);
				simm_average += simm;
			}
			simm_average = simm_average / haha;
		}
		...
		if (options.useDistance) {
			...
			if (simm_average > 0.0) {
				rep_seq_cluster_index = rep->cluster_id;
				if (updated[rep_seq_cluster_index] == 1) {
					if (simm_average < options.cluster_thd) {
						if (abs(simm_average - options.cluster_thd) > stdIS[rep_seq_cluster_index] * options.multiple)
							continue;
					}
				}
				else if (simm_average < options.cluster_thd) 
					continue;
			}
			else {
		...
		flag = 1;
		float identify_one = max(tiden_pc, simm_average);
		seq->identity = identify_one;
		seq->cluster_id = rep->cluster_id;
...
float SequenceDB::standard_deviation(Vector<float> a, float meann) {
	float aa = 0.0;
	for (int i = 0; i < a.size(); i++) {
		aa += (a[i] - meann)*(a[i] - meann);
	}
	return sqrt(aa / (a.size() - 1));
}

float SequenceDB::seq_align_my(Sequence *seq1, Sequence *seq2) {
	int gapScore = -2;
	int matchScore = 2;
	int mismatchScore = -2;
...
void SequenceDB::DoClustering(const Options & options)
{
	...
	for (i = 0; i<N; ) {
		...
		for (int ks = i; ks<m; ks++) {
			...
			i = ks + 1;
			if (seq->state & IS_REDUNDANT) continue;
			ClusterOne(seq, ks, word_table, param, buffer, options, 0);
		...
		for (int j = m; j<N; j++) {
			...
			if (seq->state & IS_REDUNDANT) continue;
			if (options.store_disk) seq->SwapIn();
			CheckOne(seq, word_table, param, buffer, options, 0);
			total_letters -= seq->size;
			if (options.store_disk && (seq->state & IS_REDUNDANT)) seq->SwapOut();
...
#pragma omp parallel for schedule( dynamic, 1 )
			for (int j = 0; j<JM; j++) {
				...
				int flag = other.CheckOne(seq, word_table, params[tid], buffers[tid], options, 1);
				if ((flag == 1) || (flag == -1)) { // if similar to old one delete it
					if (!options.cluster_best) {
						seq->Clear();
						seq->state |= IS_REDUNDANT;
			...
			for (int j = 0; j<M; j++) {
				...
				flag = other.CheckOne(seq, word_table, param, buffer, options, 1);
				if ((flag == 1) || (flag == -1)) { // if similar to old one delete it
					if (!options.cluster_best) {
						seq->Clear();
						seq->state |= IS_REDUNDANT;
